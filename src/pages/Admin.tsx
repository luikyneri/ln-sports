...
export const Admin = () => {
  ...
  useEffect(() => {
    ...
    return () => { unsubLigas(); unsubTimes(); unsubCamisas(); };
  }, []);
  ...
  const handleSaveShirt = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const selectedLeague = dbLeagues.find(l => l.id === shirtData.liga);
      const dataToSave = { 
        ...shirtData, 
  ...
  const handleSaveTeam = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const selectedLeague = dbLeagues.find(l => l.id === newTeam.league);
      const teamToSave = { 
        ...newTeam, 
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 text-slate-900 font-sans">
      <div className="max-w-6xl mx-auto">
        <header className="mb-8 font-black italic text-blue-700 uppercase text-3xl tracking-tighter">LN Sports Admin</header>
        ...
        {tab === "camisas" && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <form onSubmit={handleSaveShirt} className="bg-white p-6 rounded-2xl shadow-sm border h-fit space-y-4 sticky top-4">
              <h2 className="font-black text-blue-800 uppercase text-xs italic">{editingId ? "✏️ Editar" : "Nova Camisa"}</h2>
              <select className="w-full p-3 border rounded-xl font-bold bg-slate-50 text-xs" value={shirtData.liga} onChange={e => setShirtData({...shirtData, liga: e.target.value, time: ""})} required>
                ...
                {dbLeagues.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}
              </select>
              <select className="w-full p-3 border rounded-xl font-bold bg-slate-50 text-xs" value={shirtData.time} onChange={e => setShirtData({...shirtData, time: e.target.value})} required disabled={!shirtData.liga}>
                ...
                {dbTeams.filter(t => {
                   const leagueObj = dbLeagues.find(l => l.id === shirtData.liga);
                   return t.league === shirtData.liga || t.league === leagueObj?.name;
                }).map(t => <option key={t.id} value={t.name}>{t.name}</option>)}
              </select>
              <input placeholder="Modelo" className="w-full p-3 border rounded-xl text-xs font-bold" value={shirtData.modelo} onChange={e => setShirtData({...shirtData, modelo: e.target.value})} required />
              ...
              <div className="border-2 border-dashed p-4 rounded-xl flex flex-wrap gap-2">
                 ...
                 <label className="h-12 w-12 border-2 border-blue-200 rounded-lg flex items-center justify-center cursor-pointer font-bold text-blue-500">
                   <input type="file" className="hidden" onChange={async (e) => { const url = await uploadImage(e.target.files![0]); if (url) setShirtData(p => ({ ...p, imagens: [...p.imagens, url] })); }} />
                   {uploading ? "..." : "+"}
                 </label>
              </div>
              <button className={`w-full text-white p-4 rounded-xl font-black uppercase text-xs ${editingId ? 'bg-orange-500' : 'bg-blue-600'}`}>Salvar</button>
            </form>
            ...
            <div className="lg:col-span-2 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2 bg-white p-3 rounded-xl border shadow-sm">
                <select className="p-2 border rounded-lg text-[10px] font-black uppercase bg-slate-50" value={listFilterLeague} onChange={e => {setListFilterLeague(e.target.value); setListFilterTeam("all")}}>
                  ...
                  {dbLeagues.map(l => <option key={l.id} value={l.name}>{l.name}</option>)}
                </select>
                <select className="p-2 border rounded-lg text-[10px] font-black uppercase bg-slate-50" value={listFilterTeam} onChange={e => setListFilterTeam(e.target.value)}>
                  ...
                  {dbTeams.filter(t => listFilterLeague === "all" || t.league === listFilterLeague).map(t => <option key={t.id} value={t.name}>{t.name}</option>)}
                </select>
                <input placeholder="Buscar modelo..." className="p-2 border rounded-lg text-xs font-bold" value={searchShirt} onChange={e => setSearchShirt(e.target.value)} />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {dbShirts.filter(s => (listFilterLeague === "all" || s.liga === listFilterLeague) && (listFilterTeam === "all" || s.time === listFilterTeam) && (s.modelo?.toLowerCase().includes(searchShirt.toLowerCase()))).map(s => (
                  <div key={s.id} className="bg-white p-4 rounded-2xl border flex items-center gap-4 group">
                    ...
                    <div className="flex-1">
                      <p className="text-[10px] font-black text-blue-600 uppercase">{s.liga}</p>
                    ...
                    <div className="flex flex-col gap-2">
                      <button onClick={() => { setEditingId(s.id); setShirtData({time: s.time, liga: s.liga, modelo: s.modelo, imagens: s.imagens, preco: String(s.preco)}); }} className="p-2 text-blue-500 bg-blue-50 rounded-lg text-[10px] font-black">EDITAR</button>
                      <button onClick={() => confirm("Deletar?") && deleteDoc(doc(db, "CAMISAS", s.id))} className="p-2 text-red-500 bg-red-50 rounded-lg text-[10px] font-black">🗑️</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-6">
            <form onSubmit={handleSaveTeam} className="bg-white p-6 rounded-2xl border shadow-sm flex flex-wrap gap-4 items-end">
              <div className="flex-1 min-w-[200px]">
                <label className="text-[10px] font-black uppercase">Nome do Time</label>
                <input className="w-full p-3 border rounded-xl text-xs font-bold" value={newTeam.name} onChange={e => setNewTeam({...newTeam, name: e.target.value})} required />
              </div>
              <div className="flex-1 min-w-[200px]">
                <label className="text-[10px] font-black uppercase">Liga</label>
                <select className="w-full p-3 border rounded-xl text-xs font-bold" value={newTeam.league} onChange={e => setNewTeam({...newTeam, league: e.target.value})} required>
              ...
              <label className="p-3 border-2 border-dashed rounded-xl cursor-pointer h-[46px] flex items-center justify-center min-w-[120px]">
                <input type="file" className="hidden" onChange={async (e) => { const url = await uploadImage(e.target.files![0]); if (url) setNewTeam({...newTeam, badge: url}); }} />
                {newTeam.badge ? <img src={newTeam.badge} className="h-6" /> : <span className="text-[10px] font-black">ESCUDO +</span>}
              </label>
              <button className={`px-8 h-[46px] rounded-xl font-black text-[10px] uppercase text-white ${editingId ? 'bg-orange-500' : 'bg-slate-900'}`}>Salvar</button>
            </form>
            <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
              {dbTeams.filter(t => (teamFilterLeague === "all" || t.league === teamFilterLeague) && t.name?.toLowerCase().includes(searchTeam.toLowerCase())).map(t => (
                <div key={t.id} className="bg-white p-4 rounded-2xl border text-center relative group">
                  ...
                  <div className="absolute top-1 right-1 flex gap-1 opacity-0 group-hover:opacity-100">
                    <button onClick={() => { setEditingId(t.id); setNewTeam({name: t.name, league: t.league, badge: t.badge}); }} className="bg-blue-600 text-white p-1 rounded text-[8px]">EDIT</button>
                    <button onClick={() => deleteDoc(doc(db, "TIMES", t.id))} className="bg-red-500 text-white p-1 rounded text-[8px]">×</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {tab === "ligas" && (
          <div className="space-y-6">
            <form onSubmit={async (e) => {
              ...
              await addDoc(collection(db, "LIGAS"), { name: newLeague.name, order: dbLeagues.length });
              setNewLeague({ name: "" }); setLoading(false);
            }} className="bg-white p-6 rounded-2xl border shadow-sm flex gap-3">
              <input placeholder="Nova Liga..." className="flex-1 p-4 border rounded-xl text-xs font-bold uppercase" value={newLeague.name} onChange={e => setNewLeague({name: e.target.value})} required />
              <button className="bg-blue-700 text-white px-8 rounded-xl font-black uppercase text-xs">Criar</button>
            </form>
            <div className="bg-white p-6 rounded-2xl border shadow-sm">
              <div className="flex justify-between items-center mb-6">
                <h2 className="font-black text-slate-400 uppercase text-[10px] italic">Arraste para organizar</h2>
                {hasOrderChanges && <button onClick={saveLeaguesOrder} className="bg-green-600 text-white px-6 py-2 rounded-xl font-black text-[10px] uppercase">Salvar Ordem</button>}
              </div>
              <div className="space-y-2">
                {dbLeagues.map((l, index) => (
                  <div key={l.id} draggable onDragStart={(e) => e.dataTransfer.setData("index", index.toString())} onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleDrop(e, index)} className="flex items-center justify-between p-4 bg-slate-50 border rounded-xl cursor-move hover:bg-white transition-all">
                    <span className="text-xs font-black uppercase italic">#{index + 1} {l.name}</span>
                    <button onClick={() => confirm("Deletar?") && deleteDoc(doc(db, "LIGAS", l.id))} className="text-red-300">🗑️</button>
                  </div>
                ))}
              </div>
            </div>
